...
void main() {
  group('ActivityRecommender', () {
    ...
    test(
      'eligiblePoolSize counts unique ids, not raw entries — a duplicate '
      ...
      () {
        ...
        expect(result.eligiblePoolSize, 2, reason: '3 entries but only 2 unique ids');
      },
    );

    test(
      'eventual coverage: repeated spins present every eligible activity, '
      ...
      () {
        ...
        final catalog = [
          for (var i = 0; i < 3; i++)
            _activity(id: 'high$i', social: true, benefits: const ['physicalActivity']),
    ...
    test(
      'eventual coverage: repeats over many laps — every sliding window '
      ...
      () {
        ...
        for (var start = 0; start + windowSize <= perSpinIds.length; start++) {
          final window = perSpinIds.sublist(start, start + windowSize).expand((s) => s).toSet();
          expect(
            window,
            allIds,
            reason: 'spins $start..${start + windowSize - 1} did not cover every activity',
          );
        }
      },
    );
    ...
    test(
      'eventual coverage: cycle boundary never immediately repeats the '
        ...
        for (var spin = 0; spin < 20; spin++) {
          ...
          if (previousSpinIds != null) {
            expect(
              ...
              isEmpty,
              reason: 'spin $spin repeated an activity from the immediately previous spin',
            );
          }
    ...
    test(
      'eventual coverage: pool size not evenly divisible by pick size still '
    ...
    test(
      'eventual coverage: pool smaller than the pick size (single activity) '
      'never stalls',
      () {
